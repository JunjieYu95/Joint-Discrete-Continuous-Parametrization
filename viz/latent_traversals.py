...
class LatentTraverser():
    ...
    def _traverse_continuous_line(self, idx, size):
        ...
        if self.sample_prior:
            ...
            # original code: samples = np.zeros(shape=(size, self.cont_dim))
            # change: random sample instead of always use zero
            one_sample = np.random.normal(size=(1,self.cont_dim))
            samples = np.repeat(one_sample, size, axis = 0)
    ...
    def _traverse_discrete_line(self, dim, traverse, size):
        ...
        if traverse:
            ...
            if self.sample_prior:
                ...
                # original code: always use the first disc latent code
                # samples[:, 0] = 1.
                # modify:
                rand_int = np.random.randint(0,dim)
                samples[:, rand_int] = 1
    ...
    def _traverse_continuous_grid(self, idx, axis, size):
        ...
        if self.sample_prior:
            ...
            # modify: instead of using zero, using ranomd samples
            for i in range(size[0]):
                one_sample = np.random.normal(size=(1,self.cont_dim))
                repeat_samples = np.repeat(one_sample, size[1], axis = 0)
                samples[i*size[1]:(i+1)*size[1],:] = repeat_samples
    ...
    def _traverse_discrete_grid(self, dim, axis, traverse, size):
        ...
        for i in range(size[0]):
                rand_int = np.random.randint(0, dim)
                samples[i*size[1]:(i+1)*size[1],rand_int] = 1
